function [ce_err, ce_diff] = cross_entropy ( Wb, n, X,out_data,lambda, lda_data);
...
ce_err = -1/N*sum(sum(out_data.*log(AE_vals{end}) + (1-out_data).*log(1-AE_vals{end})));

% compute backpropagation derivatives

% initialize
delta = 1/N*((1-out_data).*AE_vals{end} - out_data.*(1-AE_vals{end}));
