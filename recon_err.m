function [re_err, re_diff] = recon_err ( Wb, n, X,out_data,lambda, lda_data);
...
%         X -- data with points as columns (input data)
%
...
AE_vals{num_maps+1} = 1./(1+exp(-W{num_maps}*AE_vals{num_maps} - ...
     repmat(b{num_maps},1,N)));
...
re_err = 1/N*sum(sum((AE_vals{end} - out_data).^2));

% compute backpropagation derivatives

% initialize
delta = 2/N*((AE_vals{end} - out_data).*AE_vals{end}.*(1-AE_vals{end}));
...
% do linear map
%delta = W{num_maps+1}'*delta+lambda*lda_data;
delta = (W{num_maps+1}'*delta+lambda*lda_data).*AE_vals{num_maps+1}.*(1-AE_vals{num_maps+1});
%delta = (W{num_maps+1}'*delta).*AE_vals{num_maps+1}.*(1-AE_vals{num_maps+1});
